...
def home_arm(motors):
    print("Moving the Arm to Home...")
    move_to_steps(0,0,0)
    print("Arm has been Homed")

def move_to_steps(motors,x,y,z):
    
    coords = [x,y,z]
    print("Moving the Arm to Target Position...")
    for motor_index in range(len(motors._PUL_devices)):
        print(motor_index)
        if motors.num_of_steps[motor_index] - coords[motor_index]> 0:
             motors.dir_motor_backward([motor_index])
             motors.set_motor_speed([motor_index], 1)
             motors.start_motor([motor_index])
        elif motors.num_of_steps[motor_index] - coords[motor_index]< 0:
             motors.dir_motor_forward([motor_index])
             motors.set_motor_speed([motor_index], 1)
             motors.start_motor([motor_index])
...
def listen_for_home_command(motors):
    ...
    while True:
        ...
        if cmd.startswith("move to"):
            try:
                numbers_str = cmd.replace("move to", "").strip()
                numbers = numbers_str.strip("[] ").split(",")
                coords = [float(num.strip()) for num in numbers]
                if len(coords) != 3:
                    print("Error: Please provide exactly 3 coordinates.")
                    continue
                pause_main_tread.clear()
                move_to_steps(motors,*coords)
                pause_main_tread.set()
            except ValueError:
                print("Invalid coordinate format. Example: Move to [12, 23, 45]")
    return exc_home_time
